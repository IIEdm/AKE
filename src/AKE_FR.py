...
print("random seed is %d, source language is %s, target language is %s, knowledge dimension is %d, epochs is %d, GAN_epochs is %d, margin is %.2f,"
      ...
      "clipping_parameter is %.3f, rbf is  %.3f. lambda_1 is % .2f, lambda_2 is %.2f, and mu is %.2f.\n "
      % (seed, lang1, lang2, kb_dimension, glob_epochs, GAN_epochs,  margin, kb_batchsize, cross_batchsize, GAN_batchsize,
         lr, cross_lr, GAN_lr,
         hidden_size, input_std, hid_std, dropout_rate, train_times, GAN_balance_factor, D_iter, clipping_parameter, rbf, lambda_1, lambda_2, mu))
...
def train_cross(cross_l1, cross_l2, generator, l1_model, l2_model):
    ...
    for l1_batch, l2_batch in sample_batch(l1_cuda, l2_cuda, cross_count):
        ...
        re_l1 = t.mm(l1_transfered, T_matrix1)
        sour_re_loss = t.sum(t.norm(l1_em - re_l1, p=2, dim=1))
        re_l2 = t.mm(l2_em, T_matrix1)
        l2_transfered = generator.transformation(re_l2)
        tar_re_loss = t.sum(t.norm(l2_em - l2_transfered, p=2, dim=1))
        batch_cross_l1_loss = lambda_1 * t.sum(t.norm(l2_em - l1_transfered, p=2, dim=1)) + lambda_2 * (mu * sour_re_loss  + (1-mu) * tar_re_loss)
